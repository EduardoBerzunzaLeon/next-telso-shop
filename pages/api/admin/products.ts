...
const updateProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        if(!product) {
            await db.disconnect();
            return res.status(400).json({ message: 'No existeun producto con ese ID' });
        }
        ...
        await db.disconnect();
        return res.status(500).json({ message: 'Ocurrio un error en el servidor, favor de contactar al administrador' });
    }
